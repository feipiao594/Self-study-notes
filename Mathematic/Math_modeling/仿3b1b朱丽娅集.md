...
void main(){
  ...
  float r = sqrt(r2);
  float nu = log(log(r)/log(ESCAPE))/log(2.0);
...
void main(){
  ...
  if(iter==MAX_ITER){ gl_FragColor = vec4(getAxisColor(setColor, x0, y0, 0.5, 0.5, 0.004),1); return; }
  ...
  vec3 color = palette(t);
  gl_FragColor = vec4(getAxisColor(color, x0, y0, 0.5, 0.5, 0.004),1);
}
`;
