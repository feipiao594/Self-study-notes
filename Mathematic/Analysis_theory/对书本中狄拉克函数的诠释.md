...
$$\lim_{a \to 0} \int_{a^-}^{a^+}$$
但是这个符号是在求线性系统的冲激响应里出现的，冲激函数作为一个广义函数来说，在严格的数学里其实应该没有定义除了整个$-\infty$到$+\infty$的和冲激函数相关的积分，而且个人觉得书中对线性系统的冲激响应求法说明不够正确，于是就有了接下来的内容
...
$$
对于这个方程，我们观察到，从逻辑上讲，解得的$h(t)$一定是一个**常义函数**，根据冲激函数的物理意义是一个瞬间的无穷大信号而言，我也可以推得在小于0的时候信号一定为0，而在大于0的时候才是我们真正需要考量的函数，因此最终在**物理意义的辅助**下我们得到了微分方程解的形式，拆解为$h(x)=f(x)u(x)$(这里的$u(x)$就是上面说的**Heaviside函数**)，而因为$h(x)$在大于0上是连续可导的函数(在大于0的部分微分方程的基本要求)，那么在小于0的部分也可以延拓出一个连续的$f(x)$，所以$f(x)$是连续可导的
